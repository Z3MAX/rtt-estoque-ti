...
const ROLE_STYLE: Record<string, string> = {
  'Administrador de TI': 'bg-primary-100 text-primary-700 border-primary-200',
  'Técnico de TI':       'bg-emerald-100 text-emerald-700 border-emerald-200',
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div>
            <h3 className="text-lg font-bold text-slate-900">{isEdit ? 'Editar Usuário' : 'Novo Usuário'}</h3>
            <p className="text-sm text-slate-500 mt-0.5">{isEdit ? 'Atualize os dados do usuário' : 'Preencha os dados do novo usuário'}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 transition-colors" disabled={loading}>
              Cancelar
            </button>
...
export default function UsersPage() {
  ...
  const [modalUser, setModalUser] = useState<AppUser | null | undefined>(undefined) // undefined = closed
  ...
  async function toggleActive(u: AppUser) {
    ...
    try {
      if (u.active) {
        ...
        await api.users.update(u.id, { active: true })
      }
      await load()
    } catch (err) {
      console.error(err)
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Gerencie contas e permissões de acesso</p>
        </div>
        {isAdmin && (
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total de usuários', value: users.length,                                     color: 'text-slate-700', bg: 'bg-slate-50 border-slate-200' },
          { label: 'Administradores',   value: users.filter(u => u.role === 'Administrador de TI').length, color: 'text-primary-700', bg: 'bg-primary-50 border-primary-200' },
          { label: 'Ativos',            value: users.filter(u => u.active).length,                color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200' },
      ...
      {/* User groups */}
      {!loading && [
        { title: 'Administradores', icon: ShieldCheck, list: admins, iconColor: 'text-primary-600', bgColor: 'bg-primary-100' },
        { title: 'Técnicos de TI',  icon: Wrench,      list: techs,  iconColor: 'text-emerald-600', bgColor: 'bg-emerald-100' },
        ...
        list.length > 0 && (
          <div key={title} className="card overflow-hidden">
            ...
            <div className="flex items-center gap-2.5 px-5 py-3.5 bg-slate-50 border-b border-slate-100">
              ...
              <span className="ml-auto text-xs font-medium text-slate-400 bg-slate-200 px-2 py-0.5 rounded-full">{list.length}</span>
            </div>

            {/* User rows */}
            <div className="divide-y divide-slate-50">
              {list.map((u) => (
                <div key={u.id} className={`flex items-center gap-4 px-5 py-4 hover:bg-slate-50/70 transition-colors ${!u.active ? 'opacity-50' : ''}`}>
                  {/* Avatar */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {u.id === currentUser?.id && (
                        <span className="text-xs bg-primary-100 text-primary-600 px-1.5 py-0.5 rounded-md font-medium">Você</span>
                      )}
                      {!u.active && (
                        <span className="text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded-md font-medium">Inativo</span>
                      )}
                    </div>
                  ...
                  {/* Role badge */}
                  <span className={`hidden sm:inline-flex text-xs font-medium px-2.5 py-1 rounded-full border ${ROLE_STYLE[u.role] ?? 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                    {u.role}
                  </span>
      ...
      {!loading && filtered.length === 0 && (
        <div className="card p-12 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-slate-400 text-sm">Tente ajustar o filtro de busca</p>
        </div>
      )}
